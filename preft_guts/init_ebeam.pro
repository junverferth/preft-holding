;------------------------------------------------------
pro apply_beam,tube
  common ebeam, amp,index,cutoff,dure,shape,h00,del,eprof,sym
  ;may wanna get clever with 3 phases, fast ramp, 2nd/slowramp/flat, decay
  if(tube.time lt del) then return
  if shape eq 1 then amplitude=(1.0-2*abs(tube.time-0.5*dure-del)/dure)>0.0
  if shape eq 0 then amplitude=1.0/dure
  if tube.time gt 1.0*dure then begin
     tube.heat=h00
     return
  endif
  ;amp=
...
if mask[0] ne -1 then beta_fun[mask]=Ibeta(delta/2.,1./3.,xc[mask])
...
pro init_ebeam, amp_, index_, cutoff_,dure_,shape_,del_,tube,sym_
  common ebeam, amp,index,cutoff,dure,shape,h00,del,eprof,sym
