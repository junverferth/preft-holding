...
;if (count1 ne 0) then begin;or (count2 ne 0) then begin
   ;dv_rest[*,a]=0.0;vector[*,a]*([1,1,1]#( spr_cnst^2*(tube.x[2,a]-zbot) - damp*total(vector[*,a]*tube.v[*,a],1))) ;for a dense loop, it snags on itself in the x direction
...
; -----------------------------
pro init_flat2,rec_ang_,b0_,ztop_,zbot_
  common cs, rec_ang,ztop,zbot,b0
common fieldshape,fieldlen,fieldstr,fieldderiv,cs_scale
